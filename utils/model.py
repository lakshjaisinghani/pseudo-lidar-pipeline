...
class bts(nn.Module):
    def __init__(self, feat_out_channels, num_features=512):
        ...
        self.max_depth = 80 # check what happens
    ...
    def forward(self, features, focal):
        ...
        #final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873 # for kitti for some reason 
...
class BtsModel(nn.Module):
    def __init__(self):
        ...
        self.bts_size = 512 # hack
        self.encoder = encoder()
        self.decoder = bts(self.encoder.feat_out_channels, self.bts_size) # removed params.bts_size

    def forward(self, x, focal):
        skip_feat = self.encoder(x)
        return self.decoder(skip_feat, focal) # sending 0 for focal